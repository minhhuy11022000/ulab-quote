...
const makeDefaultCosts = (total: number): CostLine[] => [
...
function PrintView({ items, calculated, totals, clientName }: {
  items: Item[];
  calculated: CalcRow[];
  totals: { totalCost: number; totalRev: number; profit: number; margin: number };
  clientName: string;
}) {
...
export default function App() {
  const [items, setItems] = useState<Item[]>(() => {
    try {
      const saved = localStorage.getItem('ulab-items');
  ...
  const [clientName, setClientName] = useState(() => {
    return localStorage.getItem('ulab-client') ?? 'Team Flash 2026';
  });
  ...
  useEffect(() => {
    localStorage.setItem('ulab-items', JSON.stringify(items));
  }, [items]);

  useEffect(() => {
    localStorage.setItem('ulab-client', clientName);
  }, [clientName]);

  const toggleExpand = (id: number) =>
    setExpandedRows((prev) => ({ ...prev, [id]: !prev[id] }));

  const updateItem = useCallback((id: number, field: keyof Item, val: Item[keyof Item]) => {
    setItems((prev) =>
      prev.map((it) => (it.id === id ? { ...it, [field]: val } : it)),
    );
  }, []);

  const updateCosts = useCallback((id: number, newCosts: CostLine[]) => {
    setItems((prev) =>
      prev.map((it) => (it.id === id ? { ...it, costs: newCosts } : it)),
    );
  }, []);

  const addCostLine = useCallback((id: number) => {
    setItems((prev) =>
      prev.map((it) =>
        it.id === id
  ...
  const removeCostLine = useCallback((id: number, idx: number) => {
    setItems((prev) =>
      prev.map((it) =>
        it.id === id
