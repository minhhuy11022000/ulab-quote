...
export function calcRow(item: Item, globalMargin: number): Omit<CalcRow, keyof Item> {
  ...
  const suggestedPrice = unitCost > 0 ? Math.ceil(unitCost / (1 - globalMargin) / 100) * 100 : 0;
  ...
  const isOverridden = item.priceOverride !== null;
...
export function calcQuoteTotals(quote: Quote): Totals {
  const gm = quote.globalMargin / 100;
  const t = { totalCost: 0, totalRev: 0, profit: 0, margin: 0 };
  quote.items.forEach(it => {
    const r = calcRow(it, gm);
